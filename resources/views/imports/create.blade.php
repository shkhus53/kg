<x-app-layout>
    <x-slot name="header">
        <x-shell.page-header title="Upload Duty List" :subtitle="'Session: '.$dutySession->date->format('d M Y')" :back-url="route('sessions.show', $dutySession)">
            <x-shell.steps :steps="['Upload', 'Preview', 'Changes', 'Result']" current="Upload" />
        </x-shell.page-header>
    </x-slot>
    ...
    <div class="space-y-5 lg:mx-auto lg:max-w-2xl">
        ...
        <form method="POST" action="{{ route('sessions.imports.store', $dutySession) }}" enctype="multipart/form-data" class="space-y-5"
              x-data="{ fileName: null, dragging: false, submitting: false }" @submit="submitting = true">
            @csrf

            <label for="file"
                   class="flex cursor-pointer flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed bg-white px-6 py-12 text-center transition-colors"
                   :class="dragging ? 'border-blue-400 bg-blue-50/50' : 'border-slate-300'"
                ...
                <span class="flex h-12 w-12 items-center justify-center rounded-full text-blue-500 transition-transform" :class="dragging ? 'bg-blue-100 scale-110' : 'bg-blue-50'">
                ...
                <span class="text-sm font-medium text-slate-600" x-text="fileName ?? 'Drag & Drop Excel / CSV file here'"></span>
                <span class="text-xs text-slate-400">{{ __('or') }}</span>
                <span class="kg-tap inline-flex items-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">{{ __('Choose File') }}</span>
                <input id="file" name="file" type="file" accept=".xlsx,.xls,.csv" required class="hidden" x-ref="fileInput"
            ...
            <x-shell.button tone="primary" type="submit" x-bind:disabled="submitting">
                <span x-show="!submitting">{{ __('Upload & Preview') }}</span>
