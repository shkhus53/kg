...
<aside class="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r border-slate-200 bg-white lg:flex">
    ...
    <nav class="flex-1 space-y-0.5 overflow-y-auto px-3">
        ...
        @can('import_duty_list')
            <a href="{{ route('imports.index') }}" class="kg-tap flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium {{ $navItem('imports', request()->routeIs('imports.index') || request()->routeIs('sessions.imports.*')) }}">
                <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M7 16a4 4 0 0 1-.88-7.9A5.5 5.5 0 0 1 17 8a4.5 4.5 0 0 1 .5 9H7Zm5-4v6m0-6-2.5 2.5" /></svg>
                {{ __('Import Center') }}
            </a>
        @endcan
        ...
        @canany(['view_analytics', 'view_directory'])
        ...
        @canany(['manage_masters', 'manage_users'])
            <p class="px-3 pb-1 pt-4 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{{ __('Admin') }}</p>
        @endcanany

        @can('manage_masters')
            <a href="{{ route('masters.index') }}" class="kg-tap flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium {{ $navItem('masters', request()->routeIs('masters.*')) }}">
                <svg class="h-5 w-5 shrink-0 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 21h18M5 21V7l8-4 8 4v14M9 9h1m4 0h1m-6 4h1m4 0h1m-6 4h1m4 0h1" /></svg>
                {{ __('Master Data') }}
            </a>
        @endcan

        @can('manage_users')
            <a href="{{ route('users.index') }}" class="kg-tap flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium {{ $navItem('users', request()->routeIs('users.*')) }}">
                <svg class="h-5 w-5 shrink-0 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 1 1-8 0 4 4 0 0 1 8 0Zm-8 8a6 6 0 0 0-6 6h20a6 6 0 0 0-6-6H8Z" /></svg>
                {{ __('User Management') }}
            </a>
        @endcan
    </nav>
