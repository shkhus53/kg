<x-app-layout>
    <x-slot name="header">
        <x-shell.page-header title="Edit Department" :subtitle="$department->name" :back-url="route('masters.departments.index')" />
    </x-slot>

    <div class="space-y-5">
        <x-shell.card>
            <form method="POST" action="{{ route('masters.departments.update', $department) }}" class="grid grid-cols-2 gap-3">
                @csrf
                @method('PUT')
                ...
                <div class="col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3" class="mt-1 block w-full rounded-xl border-slate-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $department->description) }}</textarea>
                </div>
                <div class="col-span-2 grid grid-cols-2 gap-3">
                    <x-shell.button tone="outline" href="{{ route('masters.departments.index') }}">{{ __('Cancel') }}</x-shell.button>
        ...
        <div>
            ...
            @php $history = app(\App\Services\MasterDataAuditService::class)->history('department', $department->id); @endphp
            @if ($history->isEmpty())
