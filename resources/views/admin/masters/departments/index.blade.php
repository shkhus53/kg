<x-app-layout>
    <x-slot name="header">
        <x-shell.page-header title="Departments" :back-url="route('masters.index')" />
    </x-slot>
    ...
    <div class="space-y-5">
        ...
        <x-shell.card>
            <h3 class="mb-3 text-sm font-semibold text-slate-700">{{ __('Add Department') }}</h3>
            <form method="POST" action="{{ route('masters.departments.store') }}" class="grid grid-cols-2 gap-3">
                @csrf
                ...
                <div class="col-span-2">
                    <x-shell.button tone="primary" type="submit">{{ __('Add Department') }}</x-shell.button>
                </div>
            </form>
        </x-shell.card>
            ...
            <div class="space-y-2 lg:grid lg:grid-cols-2 lg:gap-3 lg:space-y-0 xl:grid-cols-3">
                @foreach ($departments as $department)
                    <div class="kg-card-hover flex items-start justify-between gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                        <div class="min-w-0">
                            <p class="truncate text-sm font-medium text-slate-900">{{ $department->name }}</p>
                            <p class="text-xs text-slate-400">{{ $department->code ?: __('No code') }}</p>
                        </div>
                        <div class="flex shrink-0 items-center gap-2">
                            <x-shell.badge :tone="$department->active ? 'green' : 'gray'" dot>{{ $department->active ? __('Active') : __('Inactive') }}</x-shell.badge>
                            <a href="{{ route('masters.departments.edit', $department) }}" class="kg-tap flex h-8 w-8 items-center justify-center rounded-full text-slate-400 hover:bg-slate-50 hover:text-slate-700" aria-label="{{ __('Edit') }}">
                            ...
                            <form method="POST" action="{{ route('masters.departments.toggle', $department) }}">
                                @csrf
                                <button type="submit" class="kg-tap flex h-8 w-8 items-center justify-center rounded-full text-slate-400 hover:bg-slate-50 hover:text-slate-700" aria-label="{{ $department->active ? __('Deactivate') : __('Activate') }}">
