<x-app-layout>
    ...
    <div class="space-y-5">
        ...
        <div class="grid grid-cols-2 gap-3">
            <x-shell.button tone="primary" href="{{ route('reports.session.pdf', $dutySession) }}">
            ...
            <x-shell.button tone="outline" href="{{ route('reports.session.excel', $dutySession) }}">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v12m0 0-4-4m4 4 4-4M4 20h16" /></svg>
                {{ __('Export Excel') }}
            </x-shell.button>
        </div>
        ...
        <div class="lg:grid lg:grid-cols-5 lg:gap-3 lg:space-y-0 space-y-3">
            ...
            <x-shell.card class="lg:col-span-2">
                ...
                <p class="text-3xl font-bold tabular-nums text-emerald-600">{{ $rate !== null ? $rate.'%' : '—' }}</p>
        ...
        @if ($departments->isNotEmpty())
            <x-shell.card>
                ...
                <div class="space-y-3 lg:grid lg:grid-cols-2 lg:gap-x-6 lg:space-y-0 xl:grid-cols-3">
                    @foreach ($departments as $dept)
                        <div>
                            <div class="flex justify-between text-sm">
                                <span class="text-slate-700">{{ $dept->department_name }}</span>
                                <span class="tabular-nums text-slate-500">{{ $dept->scheduled }} {{ __('sched.') }} &middot; {{ $dept->rate }}%</span>
                            </div>
        ...
        <x-shell.card>
            ...
            @if ($assignments->isEmpty())
                <x-shell.empty-state title="{{ __('No assignments in this session') }}" />
            @else
                <div class="max-h-96 space-y-2 overflow-y-auto lg:grid lg:grid-cols-2 lg:gap-2 lg:space-y-0">
